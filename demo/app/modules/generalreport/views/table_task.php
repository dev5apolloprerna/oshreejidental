...
$tasksPriorities     = get_tasks_priorities();
    ...
    IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid=' . $this->ci->input->get('staff_id') . ')';
        ...
        array_push($where, 'AND is_lab_task = 0 AND CASE WHEN rel_type="project" AND rel_id IN (SELECT project_id FROM ' . db_prefix() . 'project_settings WHERE project_id=rel_id AND name="hide_tasks_on_main_tasks_table" AND value=1) THEN rel_type != "project" ELSE 1=1 END');
