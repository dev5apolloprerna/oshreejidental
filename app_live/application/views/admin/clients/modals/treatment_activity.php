...
<!-- Modal Contact -->
<div class="modal fade" id="appointment_activitys" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        ...
                        <div class="tw-flex">
                            ...
                            <div>
                                <h4 class="modal-title tw-mb-0">Treatment History</h4>
                            </div>
                        </div>
            </div>
...
<script>
    ...
    function show_activitys_modal(id){
        
        $('#activity-feed').html('');

         requestGetJSON("<?php echo admin_url('appointly/appointments/get_appointment_treatment_log/')?>" + id).done(function (response) {
                ...
                // Format the date with AM/PM
                   var formattedDate = date.toLocaleString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true });
                    ...
                    if(item.start_date_time != '0000-00-00 00:00:00' && item.end_date_time != '0000-00-00 00:00:00'){
                         ...
                         html += '<div class="text"><img src="' + item.profile_image + '" class="staff-profile-xs-image pull-left mright5">' + item.firstname + ' ' +  item.lastname + ' - Time Logged (' + diffInHours + ' Hours ' + diffInMinutes + ' Minutes)' + '</div>';

                       

                    } else{

                         html += '<div class="text"><img src="' + item.profile_image + '" class="staff-profile-xs-image pull-left mright5">' + item.firstname + ' ' +  item.lastname + ' - Time Logged (' + 0 + ' Hours ' + diffInMinutes + ' Minutes)' + '</div>';

                    }


                    html += '<div class="treatment"><strong>Treatment:</strong> ' + item.treatment + '</div>';
