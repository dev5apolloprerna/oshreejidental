...
<?php
/**
 * Included in application/views/admin/branches/branch.php
 */
?>
<script>
Dropzone.options.branchAttachmentsUpload = false;
var branch_id = $('input[name="branchid"]').val();
$(function() {

    if ($('#branch-attachments-upload').length > 0) {
        new Dropzone('#branch-attachments-upload', appCreateDropzoneOptions({
            paramName: "file",
    ...
    $('.branch-profile-tabs  a').not('a[href="#customer_admins"]').on('click', function() {
    ...
    $('.branch-form-submiter').on('click', function() {
    ...
    initDataTable('.table-tickets-single', admin_url + 'tickets/index/false/' + branch_id, undefined,
        undefined, 'undefined', [$('table thead .ticket_created_column').index(), 'desc']);

    /* Customer profile contracts table */
    initDataTable('.table-contracts-single-branch', admin_url + 'contracts/table/' + branch_id, undefined,
    ...
    <?php } ?>
    _table_api = initDataTable('.table-contacts', admin_url + 'branches/contacts/' + branch_id,
    ...
    initDataTable('.table-invoices-single-branch',
        admin_url + 'invoices/table/' + branch_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
    ...
    initDataTable('.table-credit-notes', admin_url + 'credit_notes/table/' + branch_id, ['undefined'], [
        'undefined'
    ], undefined, [0, 'desc']);

    /* Customer profile Estimates table */
    initDataTable('.table-estimates-single-branch',
        admin_url + 'estimates/table/' + branch_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
    ...
    initDataTable('.table-payments-single-branch',
        admin_url + 'payments/table/' + branch_id, undefined, undefined,
        'undefined', [0, 'desc']);

    /* Customer profile reminders table */
    initDataTable('.table-reminders', admin_url + 'misc/get_reminders/' + branch_id + '/' + 'customer',
        undefined, undefined, undefined, [1, 'asc']);

    /* Customer profile expenses table */
    initDataTable('.table-expenses-single-branch',
        admin_url + 'expenses/table/' + branch_id,
        'undefined',
        'undefined',
        'undefined', [6, 'desc']);

    /* Customer profile proposals table */
    initDataTable('.table-proposals-branch-profile',
        admin_url + 'proposals/proposal_relations/' + branch_id + '/customer',
        'undefined',
        'undefined',
        'undefined', [6, 'desc']);

    /* Custome profile projects table */
    initDataTable('.table-projects-single-branch', admin_url + 'projects/table/' + branch_id, undefined,
        undefined, 'undefined',
    ...
    appValidateForm($('.branch-form'), vRules);
    ...
    if (typeof(branch_id) == 'undefined') {
        $('#company').on('blur', function() {
            ...
            $.post(admin_url + 'branches/check_duplicate_customer_name', {
                    company: company
                })
    ...
    $('.branch-copy-billing-address').on('click', function(e) {
        e.preventDefault();
...
function delete_contact_profile_image(contact_id) {
    requestGet('branches/delete_contact_profile_image/' + contact_id).done(function() {
...
function saveCustomerProfileExternalFile(files, externalType) {
    $.post(admin_url + 'branches/add_external_attachment', {
        files: files,
        branchid: branch_id,
...
function contact(branch_id, contact_id) {
    if (typeof(contact_id) == 'undefined') {
        contact_id = '';
    }
    requestGet('branches/form_contact/' + branch_id + '/' + contact_id).done(function(response) {
...
function update_all_proposal_emails_linked_to_contact(contact_id) {
    ...
    $.post(admin_url + 'branches/update_all_proposal_emails_linked_to_customer/' + contact_id, data).done(function(
        response) {
        response = JSON.parse(response);
        if (response.success) {
            alert_float('success', response.message);
...
function do_share_file_contacts(edit_contacts, file_id) {
    ...
    $.post(admin_url + 'branches/update_file_share_visibility', {
        file_id: file_id,
        share_contacts_id: contacts_shared_ids_selected,
        branch_id: $('input[name="branchid"]').val()
    }).done(function() {
        window.location.reload();
    });
}

function save_longitude_and_latitude(branchid) {
    var data = {};
    data.latitude = $('#latitude').val();
    data.longitude = $('#longitude').val();
    $.post(admin_url + 'branches/save_longitude_and_latitude/' + branchid, data).done(function(response) {
        if (response == 'success') {
            alert_float('success', "<?php echo e(_l('updated_successfully', _l('branch'))); ?>");
        }
        setTimeout(function() {
            window.location.reload();
        }, 1200);
