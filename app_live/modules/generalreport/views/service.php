...
">
            <div class="col-2 col-sm-2" style="margin-top:8px;">
                 ...
                 <div class="col-1 col-sm-1" style="margin-top:8px;">
                    <b style="font-size:15px;"> Receipt </b>
                 </div>
                 </div>
          <?php 
                  $date=date('Y-m-d');
                    ...
                    if($servicedetails['customer_prefered_datetime'] != '' && $servicedetails['customer_prefered_datetime'] != '0000-00-00'){
                     ...
                     //foreach ($servicesdetails as $key => $value) {
                      ...
                      ?>
                        ...
                        <?php if($servicedetails['challanid'] != 0 && $servicedetails['service_plan_id'] != 0){
                            $tag = 'FREE';
                        }
                        elseif($servicedetails['service_plan_id'] != 0 && $servicedetails['challanid'] == 0){
                            $tag = 'AMC';
                        }else{
                            $tag = 'OD';
                        }

                        echo '<span class="label label-tag tag-id-0"    style="pointer-events: none !important;    margin-top: 0px;" ><span class="tag">'.$tag.'</span><span class="hide"></span></span>';?>
                 ...
                 <div class="col-1 col-sm-1" style="margin-top:8px;">
                    ...
                    <?php 
                    $CI = &get_instance();
                    $CI->db->from(db_prefix() . 'service_receipt');
                    $CI->db->where(db_prefix() . 'service_receipt' . '.service_schedule_id', $servicedetails['service_schedule_id']);
                    $service_receipt = $CI->db->get()->row();
                    $receipt_url     = base_url('api/mechanic/pdf?id=' . $servicedetails['service_schedule_id'] . '&output_type=I');
                    
                   // echo !empty($service_receipt) ? '<a href="'.base_url('api/mechanic/pdf?id='.$servicedetails['service_schedule_id'].'&output_type=I').'" target="_blank">View</a>' : '';

                    if (!empty($service_receipt)) {
                        echo '<a href="' . $receipt_url . '" target="_blank">View</a>';
                        echo '<br><a href="' . $receipt_url . '&regenerate=1" target="_blank">Regenerate</a>';
                    } else {
                        echo '<a href="' . $receipt_url . '&regenerate=1" target="_blank">Generate</a>';
                    }
