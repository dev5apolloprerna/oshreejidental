...
-- ----------------------------------------------------------------------------
-- delete the duplicate prescription's own medicine lines first
DELETE pi FROM tblappointment_prescription_items pi
JOIN tblappointment_prescriptions t2 ON t2.id = pi.prescription_id
JOIN tblappointment_prescriptions t1
  ON t1.branch_id      = t2.branch_id
...
-- then delete the duplicate prescription itself
DELETE t2 FROM tblappointment_prescriptions t1
...
-- ----------------------------------------------------------------------------
-- delete medical_history rows belonging to the duplicate client copy (run first,
-- while tblclients/tblcontacts are still intact so the relation checks below work)
DELETE mh FROM tblmedical_history mh
JOIN tblclients t2 ON t2.userid = mh.userid
JOIN tblclients t1
  ON t1.branch_id = t2.branch_id
...
-- delete the duplicate contact row (run second, while tblclients is still intact)
DELETE c FROM tblcontacts c
JOIN tblclients t2 ON t2.userid = c.userid
JOIN tblclients t1
  ON t1.branch_id = t2.branch_id
 AND t1.userid < t2.userid
 AND MD5(CONCAT_WS('|', t1.company, t1.vat, t1.phonenumber, t1.country, t1.city, t1.zip, t1.state,
...
-- finally delete the duplicate client itself (run last)
DELETE t2 FROM tblclients t1
JOIN tblclients t2
  ON t1.branch_id = t2.branch_id
 AND t1.userid < t2.userid
 AND MD5(CONCAT_WS('|', t1.company, t1.vat, t1.phonenumber, t1.country, t1.city, t1.zip, t1.state,
...
WHERE t1.branch_id IN (1,2,3)
  ...
  AND NOT EXISTS (SELECT 1 FROM tblevents        ev WHERE ev.userid   = t2.userid)
  AND NOT EXISTS (SELECT 1 FROM tblcontact_permissions cp WHERE cp.userid = t2.userid);
  -- (tblcontacts for this userid is already deleted by the previous statement,
  --  so no further contact-based relation check is needed here)
...
-- ----------------------------------------------------------------------------
DELETE t2 FROM tblmedical_history t1
