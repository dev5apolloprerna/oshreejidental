...
class Clients_model extends App_Model
{
    ...
    public function get_medicaldata($id)
    {
        
        $this->db->where('userid',$id);

        return $this->db->get(db_prefix() . 'medical_history')->row();
    }

    public function get_xrayfiles($id)
    {
    ...
    // public function next_paitent_number()
    // {
    //     $MAIN_DB = $this->load->database('default', TRUE);
    //     $MAIN_DB->where('name', 'next_patient_number');
    //     return $MAIN_DB->get(db_prefix() . 'options')->row();
    ...
    {    
        ...
        // $client_id = $this->db->insert_id();
        // $medical_history['userid '] = $client_id;
        ...
        $medical_history = [
            ...
            'immediate_text' => $data['immediate_text'] != '' ? $data['immediate_text'] : '',
            'planned_text' => $data['planned_text'] != '' ? $data['planned_text'] : '',
        ];
        ...
        // }
        if (isset($data['treatment_plan']) && is_array($data['treatment_plan'])) {
            $medical_history['treatment_plan'] = implode(', ', $data['treatment_plan']);
        }else if(isset($data['treatment_plan']) && !is_array($data['treatment_plan'])){
            $medical_history['treatment_plan'] = $data['treatment_plan'];
        } else{
            $medical_history['treatment_plan'] = '';
        }
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        $this->db->where('userid', $id);
        $this->db->update(db_prefix() . 'clients', array_merge($company_data, [
            'datecreated' => date('Y-m-d H:i:s'),
        ]));
        ...
        $this->db->update(db_prefix() . 'contacts', array_merge($contact_data, [
            'datecreated' => date('Y-m-d H:i:s'),
        ]));
        ...
        $this->db->where('userid',$id);
        $medical = $this->db->update(db_prefix() . 'medical_history', array_merge($medical_history, [
            'datecreated' => date('Y-m-d H:i:s'),
     ...
     * @param boolean $not_manual_request is manual from admin area customer 
     * or register, convert to lead
     */
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('userid', $id);
            $this->db->delete(db_prefix() . 'medical_history');
        }
        if ($affectedRows > 0) {
