...
foreach ($tables as $tableName) {
    $createCommand = $CI->db->query("SHOW CREATE TABLE `{$sourceDbName}`.`{$tableName}`")->fetchColumn(1);
    $carefulCreateCommand = str_replace("CREATE TABLE", "CREATE TABLE IF NOT EXISTS", $createCommand);
    
    $CI->db->exec($carefulCreateCommand);
    echo "Table `{$tableName}` created" . PHP_EOL;
  
    $CI->db->exec("INSERT INTO `{$destinationDbName}`.`{$tableName}` SELECT * FROM `{$sourceDbName}`.`{$tableName}`");
