...
function appointly_register_menu_items()
{
    ...
    if (staff_can('view', 'appointments') || staff_can('view_own', 'appointments')) {
        ...
        $CI->app_menu->add_sidebar_children_item(APPOINTLY_MODULE_NAME, [
            'slug'     => 'appointly-inperson',
            'name'     => 'appointment_menu_inperson_form_link',
            'href'     => site_url('appointly/inperson/form?col=col-md-8+col-md-offset-2'),
            'href_attributes' => 'target="_blank" rel="noopener noreferrer"',
            'position' => 30,
            'icon'     => 'fa-brands fa-wpforms',
        ]);
...
function get_items_table_data_new($transaction, $type, $for = 'html', $admin_preview = false)
{
    include_once(FCPATH .'modules/'.APPOINTLY_MODULE_NAME.'/libraries/App_items_table.php');
