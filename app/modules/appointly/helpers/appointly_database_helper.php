...
if (!function_exists('init_appointly_database_tables')) {
    ...
    function init_appointly_database_tables()
    {
        ...
        $tbl = db_prefix() . 'appointly_appointments';

        $CI->db->query("
            ALTER TABLE `$tbl`
            ADD COLUMN `gender` ENUM('male','female') NOT NULL DEFAULT 'male' AFTER `last_recurring_date`,
            ADD COLUMN `age` VARCHAR(100) NOT NULL DEFAULT '' AFTER `gender`
        ");
