...
if($this->ci->input->get('staff_id') != ''){
    $filters[] = 'AND ' . db_prefix() . 'appointly_appointments.id
    IN (SELECT appointment_id FROM ' . db_prefix() . 'appointly_attendees WHERE staff_id=' . $this->ci->input->get('staff_id') . ')';
}

if ($this->ci->input->get('status') == 'approved') {
    $filters[] = 'AND approved = 1';
}
if ($this->ci->input->get('status') == 'cancelled') {
    $filters[] = 'AND cancelled = 1';
}
if ($this->ci->input->get('status') == 'finished') {
    $filters[] = 'AND finished = 1';
}
if ($this->ci->input->get('internal')) {
    $filters[] = 'AND (source= "internal")';
}
if ($this->ci->input->get('external')) {
    $filters[] = 'AND (source= "external")';
}
if ($this->ci->input->get('lead_related')) {
    $filters[] = 'AND (source= "lead_related")';
}
if ($this->ci->input->get('internal_staff')) {
    $filters[] = 'AND (source= "internal_staff_crm")';
}
if ($this->ci->input->get('status') == 'finished') {
    $filters[] = 'AND finished = 1';
}
if ($this->ci->input->get('status') == 'not_approved') {
    $filters[] = 'AND approved != 1';
}
if ($this->ci->input->get('status') == 'upcoming') {
    $filters[] = 'AND date > CURDATE()';
}
if ($this->ci->input->get('status') == 'missed') {
    $filters[] = 'AND date < CURDATE()';
}
if ($this->ci->input->get('recurring')) {
...
if($client_id != ''){
    ...
    $filters[] = 'AND  contact_id = '.$contact->id;
...
//     if($sql != ''){
//         $sql .= ' AND delivery_man = '.$deliverymans;
//     }else{
//         $sql .= ' WHERE  delivery_man = '.$deliverymans;
//     }
// }
