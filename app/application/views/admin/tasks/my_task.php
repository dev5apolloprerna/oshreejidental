...
echo form_open_multipart(admin_url('tasks/task' . ($id ? '/' . $id : '')), ['id' => 'task-form']); ?>
<style>
#new-task-attachments hr{
    display: none;        
}
</style>
<div class="modal fade<?php if (isset($task)) {
                        ...
                        <div class="row">
                            ...
                            if ($rel_type == 'customer') {
                                echo 'selected';
                                        ...
                                        <option value="customer" data-select_type ="labwork" <?php if (isset($task) || $this->input->get('rel_type')) {
                            if ($rel_type == 'customer' && $task->is_lab_task == 1) {
                                echo 'selected';
                            }
                        } ?>>
                                            <?php echo _l('Lab Work'); ?>
                                        </option>
                                        <?php
                            ...
                            <div class="col-md-6">
                                ...
                                echo '<option value="' . $rel_val['id'] . '" selected>' . $rel_val[''] . '</option>';
                            } ?>
                                        </select>
                                    </div>
                                </div>
    ...
    <div class="form-group<?php if (!$task) {
        echo ' hide';
    } ?>" id="receiver_wrapper">
        <label for="receiver" class="control-label">Receiver</label>
        <div id="receiver_select">
            <select name="receiver" id="receiver" class="selectpicker" data-width="100%" 
                data-live-search="true"
                data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                <?php
                // Generate options for the select field
                foreach ($members as $member) {
                    $selected = ($task->receiver == $member['staffid']) ? 'selected' : '';
                    echo '<option value="' . $member['staffid'] . '" ' . $selected . '>' . $member['firstname'] . ' ' . $member['lastname'] . '</option>';
                }
                ?>
            </select>
        </div>
                             ...
                             <!-- <div class="col-md-6">
                                <div class="form-group<?php if ($rel_id == '') {
                                echo '';
                            } ?> appointment_options"  id="appoint_wrapper" style="display: none;">
                                    <label for="rel_id" class="control-label"><span class="">Appointments</span></label>
                                    <div id="rel_id_select">
                                        <select name="appointment_id" id="appointment_id" class="selectpicker" data-width="100%"
                                            data-live-search="true"
                                            data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                             <option value=""></option>
                        ...
                        <?php if (!isset($task)) { ?>
                        <div class="row">
                            <div class="col-md-6" id="assignees_select">
                             ...
                             <div class="col-md-6" id="lab_assti_select" style="display:none;">
                                <div class="form-group select-placeholder>">
                                    <label for="assignees"><?php echo _l('task_single_assignees'); ?></label>
                                    <select name="lab_assistant[]" id="lab_assistant" class="selectpicker" data-width="100%"
                                        data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>"
                                        multiple data-live-search="true">
                                        <?php foreach ($members as $member) { 
                                             if($member['role'] == 2){?>
                     ...
                     $receiver = (get_option('new_task_auto_follower_current_member') == '1') ? [get_staff_user_id()] : '';
                     echo render_select('receiver', $filtered_members, ['staffid', ['firstname', 'lastname']], 'Receiver', $receiver);
                     ?>
                            </div>
               ...
               echo render_textarea('description', '', (isset($task) ? $task->description : ''), ['rows' => 6, 'placeholder' => _l('task_add_description'), 'data-task-ae-editor' => true, !is_mobile() ? 'onclick' : 'onfocus' => (!isset($task) || isset($task) && $task->description == '' ? 'init_editor(\'.tinymce-task\', {height:200, auto_focus: true});' : '')], [], 'no-mbot', 'tinymce-task'); ?>
                ...
                <?php if (!isset($task)) { ?>
                        <a href="#" class="tw-pt-2 btn btn-primary pull-left new"
                            onclick="slideToggle('#new-task-attachments'); return false;" style="margin-top: 20px;margin-bottom: 20px;">
    ...
    $(function() {
        ...
        var txtrel = _rel_type.find('option:selected').text();

        if(_rel_type.find('option:selected').data('select_type') == 'labwork'){
            var txtrel = 'Patient';
        }

        $('.rel_id_label').html(txtrel);
        $('.rel_id_label').html(txtrel);
        ...
        _rel_type.on('change', function() {
            ...
            if($(this).find(':selected').data('select_type') == 'labwork'){

                //document.getElementById('appoint_wrapper').style = 'display:block;';
                //document.getElementById('rel_id_wrapper').style = 'display:none;';
                document.getElementById('assignees_select').style = 'display:none;';
                document.getElementById('lab_assti_select').style = 'display:block;';
                document.getElementsByClassName('rel_id_label')[0].innerHTML = 'Patient';

                
            
            }else{
                
                //document.getElementById('appoint_wrapper').style = 'display:none;';
                //document.getElementById('rel_id_wrapper').style = 'display:block;';
                document.getElementById('assignees_select').style = 'display:block;';
                document.getElementById('lab_assti_select').style = 'display:none;';
                document.getElementsByClassName('rel_id_label')[0].innerHTML = 'Patient';
            }
