    ...
    ->outputUsing(function ($params) {
        ...
        foreach ($rResult as $aRow) {
            ...
            $row[] = $aRow['userid'];
             ...
             $uid = '<a href="' . $url . '">' . $aRow['uid'] . '</a>';

             $uid .= '<div class="row-options">';
            $uid .= '<a href="' . admin_url('clients/client/' . $aRow['userid'] . ($isPerson && $aRow['contact_id'] ? '?group=contacts' : '?group=patient_profile')) . '">' . _l('view') . '</a> | <a href="' . admin_url('clients/client/' . $aRow['userid'] .'?group=profile') . '">' . _l('edit') . '</a>';

            if ($aRow['registration_confirmed'] == 0 && is_admin()) {
                $uid .= ' | <a href="' . admin_url('clients/confirm_registration/' . $aRow['userid']) . '" class="text-success bold">' . _l('confirm_registration') . '</a>';
            }
