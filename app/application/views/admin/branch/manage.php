                    ...
                    <div class="panel-body">
                      ...
                      if (staff_cant('view', 'customers')) {
                            ...
                            <div class="tw-grid tw-grid-cols-2 md:tw-grid-cols-3 lg:tw-grid-cols-6 tw-gap-2">
                                <div
                                    ...
                                    <span class="tw-font-semibold tw-mr-3 rtl:tw-ml-3 tw-text-lg">
                                        <?php echo total_rows(db_prefix() . 'branch', ($where_summary != '' ? substr($where_summary, 5) : '')); ?>
                                    </span>
                                    <span
                                ...
                                <div
                                    class="md:tw-border-r md:tw-border-solid md:tw-border-neutral-300 tw-flex-1 tw-flex tw-items-center">
                                    <span class="tw-font-semibold tw-mr-3 rtl:tw-ml-3 tw-text-lg">
                                        <?php echo total_rows(db_prefix() . 'branch', 'active=1' . $where_summary); ?></span>
                                    <span
                                ...
                                <div
                                    class="md:tw-border-r md:tw-border-solid md:tw-border-neutral-300 tw-flex-1 tw-flex tw-items-center">
                                    <span class="tw-font-semibold tw-mr-3 rtl:tw-ml-3 tw-text-lg">
                                        <?php echo total_rows(db_prefix() . 'branch', 'active=0' . $where_summary); ?></span>
                                    <span
                                            ...
                                            $contactsTemplate = '';
                                                if (count($contacts_logged_in_today) > 0) {
                                                    foreach ($contacts_logged_in_today as $contact) {
                                                        $url          = admin_url('branches/branch/' . $contact['userid'] . '?contactid=' . $contact['id']);
                        ...
                        <a href="#" data-toggle="modal" data-target="#customers_bulk_action"
                            ...
                            data-table=".table-branches"><?php echo _l('bulk_actions'); ?></a>
                     ...
                     $table_data  = [];
                     $_table_data = [
                      '<span class="hide"> - </span><div class="checkbox mass_select_all_wrap"><input type="checkbox" id="mass_select_all" data-to-table="branches"><label></label></div>',
                       [
                         'name'     => _l('the_number_sign'),
                         'th_attrs' => ['class' => 'toggleable', 'id' => 'th-number'],
                        ],
                         [
                         'name'     => _l('branch_list_company'),
                     ...
                     ?>
                        <div class="panel-table-full">
                            <?php
                                render_datatable($table_data, 'branches', ['number-index-2'], [
...
<script>
$(function() {
    var tAPI = initDataTable('.table-branch', admin_url + 'branches/table', [0], [0], {},
...
function customers_bulk_action(event) {
    ...
    if (r == false) {
        ...
        var rows = $('.table-branches').find('tbody tr');
        $.each(rows, function() {
        ...
        setTimeout(function() {
            $.post(admin_url + 'branches/bulk_action', data).done(function() {
