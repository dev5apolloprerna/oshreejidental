...
function handle_client_attachments_upload($id, $customer_upload = false)
{
    ...
    if (!empty($_FILES['file']['name'])) {
        ...
        // Check if temporary file path and file extension are valid
        if (!empty($tmpFilePath) && !empty($file_extension)) {
            // Construct new file name and path
            $originalFileName = pathinfo($_FILES['file']['name'], PATHINFO_FILENAME);
            if (in_array(strtolower($file_extension), ['jpg', 'jpeg', 'png', 'gif'])) {
                // For image files, use "IMG_timestamp" format
                $newFileName = 'IMG_' . time() . '.' . $file_extension;
            } else {
                // For non-image files, use the actual file name with a timestamp
                $newFileName = $originalFileName . '_' . time() . '.' . $file_extension;
            }
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                if (is_image($newFilePath)) {
                    if (!_resize_image($newFilePath, $file_extension)) {
                        // If resizing failed, delete the uploaded file
                        unlink($newFilePath);
                        return false; // Return false indicating failure
                    }
                }

                // Additional actions if it's a customer upload
                $attachment = [
                    'file_name' => $newFileName,
                    'filetype'  => $_FILES['file']['type'],
                    'xray_title'  => isset($_POST['xray_title']) ? $_POST['xray_title'] : '', 
                ];

                if ($customer_upload) {
                    $attachment['staffid'] = 0;
                    $attachment['contact_id'] = get_contact_user_id();
...
function _resize_image($filePath, $extension, $quality = 70)
{
    ...
    // Calculate new dimensions here (e.g., scaling)
    $newWidth = $width; // Example: $width / 2;
    $newHeight = $height; // Example: $height / 2;
    ...
    switch (strtolower($extension)) {
        ...
        case 'png':
            imagepng($resizedImage, $filePath, floor($quality / 10)); // Quality range for PNG is 0 - 9
            break;
